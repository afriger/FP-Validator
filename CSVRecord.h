...
public:
	...
	CSVRecord(const CSVRecord& rec)
			: m_record(rec.m_record), m_data(rec.m_data)
	{
		pLogger = Logger::getInstance();
	}
	CSVRecord(const std::vector<std::string> & rec)
			: m_record(rec)
	{
		InitTargetData();
	}
	CSVRecord& operator =(const CSVRecord& rec)
	{
...
TargetData CSVRecord::GetTargetData(double Value)
{
